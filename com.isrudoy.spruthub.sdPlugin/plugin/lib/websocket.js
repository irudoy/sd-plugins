/**
 * WebSocket communication for Sprut.Hub Plugin
...
// Note: StreamDockMessage type is imported from the root types folder
...
function initWebSocket(port, uuid, registerEvent, onMessage) {
  log('[SprutHub] Starting with port:', port, 'uuid:', uuid);

  websocket = new WebSocket(`ws://127.0.0.1:${port}`);

  websocket.on('open', () => {
    log('[SprutHub] WebSocket connected');
    if (!websocket) return;
    websocket.send(
      JSON.stringify({
        event: registerEvent,
  ...
  websocket.on('error', (/** @type {Error} */ error) => {
    log('[SprutHub] WebSocket error:', error);
  });

  websocket.on('close', () => {
    log('[SprutHub] WebSocket closed');
  });
