...
function isOutletService(service) {
  return service.type === SERVICE_OUTLET || service.type === 47;
}

function getOutletServices(accessory) {
  return accessory.services?.filter(isOutletService) || [];
}
...
function populateServices() {
  ...
  const outletServices = getOutletServices(accessory);
  if (outletServices.length === 0) {
    hideServiceSelect();
    return;
  }
  if (outletServices.length === 1) {
    hideServiceSelect();
    selectServiceById(outletServices[0].sId);
    return;
  }
  showServiceSelect();
  ...
  outletServices.forEach((service) => {
    const option = document.createElement('option');
    option.value = String(service.sId);
    option.textContent = service.name || `Outlet ${service.sId}`;
    select.appendChild(option);
  });
  if (typeof $settings !== 'undefined' && $settings && $settings.serviceId) {
    const savedServiceId = String($settings.serviceId);
    if (outletServices.some((s) => s.sId === parseInt(savedServiceId)))
