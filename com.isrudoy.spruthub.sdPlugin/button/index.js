...
const $dom = {
  ...
  pressTypeSelect: $('#pressTypeSelect'),
...
function getButtonServices(accessory) {
  return accessory.services?.filter(isButtonService) || [];
}
...
function findProgrammableSwitchEventCharacteristic(service) {
...
function populateServices() {
  ...
  const buttonServices = getButtonServices(accessory);
  ...
  if (buttonServices.length === 1) {
    hideServiceSelect();
    selectServiceById(buttonServices[0].sId);
    return;
  }
  ...
  buttonServices.forEach((service) => {
    const option = document.createElement('option');
    option.value = String(service.sId);
    option.textContent = service.name || `Button ${service.sId}`;
    select.appendChild(option);
  });

  if (typeof $settings !== 'undefined' && $settings && $settings.serviceId) {
    const savedServiceId = String($settings.serviceId);
    if (buttonServices.some((s) => s.sId === parseInt(savedServiceId))) {
...
function selectServiceById(serviceId) {
  ...
  const eventChar = findProgrammableSwitchEventCharacteristic(service);
  if (!eventChar) return;

  if (typeof $settings !== 'undefined' && $settings) {
    $settings.serviceId = service.sId;
    $settings.serviceName = service.name;
    $settings.characteristicId = eventChar.cId;
  }
...
function loadSettings(settings) {
  ...
  if (settings.pressType !== undefined && $dom.pressTypeSelect) {
    $dom.pressTypeSelect.value = String(settings.pressType);
  }
}
...
function saveSettings() {
  ...
  $settings.pressType = parseInt($dom.pressTypeSelect?.value || '0');
