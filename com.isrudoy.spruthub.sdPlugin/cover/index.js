...
function getCoverServices(accessory) {
  return accessory.services?.filter(isCoverService) || [];
}
...
function findPositionCharacteristics(service) {
...
function populateServices() {
  ...
  const coverServices = getCoverServices(accessory);
  if (coverServices.length === 0) {
    hideServiceSelect();
    return;
  }
  if (coverServices.length === 1) {
    hideServiceSelect();
    selectServiceById(coverServices[0].sId);
    return;
  }
  showServiceSelect();
...
function selectServiceById(serviceId) {
  ...
  if (!service) return;
  const { currentPosition, targetPosition } = findPositionCharacteristics(service);
  if (!targetPosition) return;
  if (typeof $settings !== 'undefined' && $settings) {
    $settings.serviceId = service.sId;
    $settings.serviceName = service.name;
    $settings.targetPositionCharId = targetPosition.cId;
    $settings.currentPositionCharId = currentPosition?.cId;
  }
  saveSettings();
}
...
function sendSettingsToPlugin() {
  ...
  $websocket.sendToPlugin({
    ...
    targetPositionCharId: $settings.targetPositionCharId,
    currentPositionCharId: $settings.currentPositionCharId,
