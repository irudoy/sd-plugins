/**
 * WebSocket communication for A Control Plugin
...
function initWebSocket(port, uuid, registerEvent, onMessage) {
  log('[AControl] Starting with port:', port, 'uuid:', uuid);

  websocket = new WebSocket(`ws://127.0.0.1:${port}`);

  websocket.on('open', () => {
    log('[AControl] WebSocket connected');
    if (!websocket) return;
    websocket.send(
      JSON.stringify({
        event: registerEvent,
  ...
  websocket.on('error', (/** @type {Error} */ error) => {
    log('[AControl] WebSocket error:', error);
  });

  websocket.on('close', () => {
    log('[AControl] WebSocket closed');
  });
