/**
 * Razer battery detection for Win Tools Plugin
 *
 * Uses native Windows HID helper - no node-hid dependency, no mouse blocking.
...
/** @type {string} */
const HELPER_PATH = path.join(__dirname, 'razer-battery-helper.exe');
...
/** @type {{devices: RazerDeviceInfo[], timestamp: number}|null} */
let enumerateCache = null;

/** @type {number} */
const ENUMERATE_CACHE_TTL = 5000;
...
function isHelperAvailable() {
  ...
  try {
    helperAvailable = fs.existsSync(HELPER_PATH);
    log(
...
function callHelper(args) {
  log(`[Razer] callHelper: ${HELPER_PATH} ${args.join(' ')}`);
  return new Promise((resolve, reject) => {
    execFile(HELPER_PATH, args, { timeout: 5000 }, (error, stdout, stderr) => {
      if (stderr) log(`[Razer] helper stderr: ${stderr.trim()}`);
      if (error) {
        log(`[Razer] helper error: ${error.message}`);
        reject(new Error(error.message));
        return;
      }

      log(`[Razer] helper stdout: ${stdout.trim()}`);
      try {
...
async function getRazerDevices() {
  ...
  if (enumerateCache && Date.now() - enumerateCache.timestamp < ENUMERATE_CACHE_TTL) {
    return enumerateCache.devices;
  }
  ...
  try {
    ...
    enumerateCache = { devices, timestamp: Date.now() };
    return devices;
  } catch (e) {
...
function getCachedRazerBattery(deviceName) {
  const now = Date.now();
  for (const [devicePath, cached] of batteryCache.entries()) {
    // Check expiration
    if (now - cached.timestamp > CACHE_MAX_AGE) {
      batteryCache.delete(devicePath);
      continue;
    }
